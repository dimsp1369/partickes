...
Particle::Particle(RenderTarget &target, int numPoints, Vector2i mouseClickPosition) : m_A(2, numPoints) {
    ...
    m_vy = (rand() % 401 + 100) * (rand() % 2 == 0 ? -1 : 1);

    m_color1 = Color::White;
    m_color2 = Color(rand() % 256, rand() % 256, rand() % 256);

    srand(static_cast<unsigned int>(time(0)));
    G = static_cast<float>(rand() % 1000 + 1);
...
void Particle::draw(RenderTarget &target, RenderStates states) const {
    ...
    for (int j = 1; j <= m_numPoints; ++j) {
        Vector2f point = Vector2f(m_A(0, j - 1), m_A(1, j - 1));
        Vector2i pixelPos = target.mapCoordsToPixel(point, m_cartesianPlane);
        lines[j].position = Vector2f((float) pixelPos.x, (float) pixelPos.y);
        lines[j].color = m_color2;

        // Glow Effect
        CircleShape particle(2);
        particle.setPosition(lines[j].position);
        particle.setFillColor(m_color2);
